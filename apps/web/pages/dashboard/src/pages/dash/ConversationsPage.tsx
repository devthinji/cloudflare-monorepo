        ...
        <TabsContent value="users" className="mt-4">
          {customers.length === 0 ? (
            ...
            <div className="divide-y rounded-md border">
              {customers.map(c => (
                <div key={c.id} className={`flex items-center justify-between p-3 cursor-pointer hover:bg-gray-50 ${selectedUserId === c.id ? 'bg-blue-50' : ''}`} onClick={() => selectUser(c.id)}>
                  <div>
                  ...
                  <Badge className={c.registered ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}>
        ...
        <TabsContent value="conversations" className="mt-4">
          {loadingConv ? (
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="divide-y rounded-md border">
                {conversations.map(c => (
                  <div key={c.id} className={`p-3 cursor-pointer hover:bg-gray-50 ${selectedConvId === c.id ? 'bg-blue-50' : ''}`} onClick={() => selectConversation(c.id)}>
                    ...
                    <p className="text-xs text-muted-foreground">{c.messageCount ?? 0} messages · {new Date(c.createdAt).toLocaleDateString()}</p>
                  </div>
                ))}
              </div>
              <div className="rounded-md border p-3 max-h-[500px] overflow-y-auto">
                {messages.length === 0 ? (
                  <p className="text-sm text-muted-foreground text-center py-8">Select a conversation to view messages.</p>
                ) : (
                  <div className="space-y-3">
                    {messages.map(m => (
                      <div key={m.id} className={`p-2 rounded-lg text-sm ${m.role === 'assistant' ? 'bg-blue-50 ml-4' : 'bg-gray-50 mr-4'}`}>
