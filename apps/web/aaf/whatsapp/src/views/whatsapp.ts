...
export class WhatsappView {
  ...
  async sendHelp(phoneNumberId: string, to: string) {
    await sendHelp(phoneNumberId, to, this.env.WHATSAPP_TOKEN)
  }

  async sendReset(phoneNumberId: string, to: string) {
    await sendReset(phoneNumberId, to, this.env.WHATSAPP_TOKEN)
  }

  async sendError(phoneNumberId: string, to: string) {
    await sendError(phoneNumberId, to, this.env.WHATSAPP_TOKEN)
  }

  async sendReply(phoneNumberId: string, to: string, reply: string) {
    await sendReply(phoneNumberId, to, reply, this.env.WHATSAPP_TOKEN)
  }
}
