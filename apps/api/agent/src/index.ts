...
// POST /api/v1/agent/chat  { agentSlug, userId, message, channel? }
app.post('/api/v1/agent/chat', async (c) => {
  const log  = createLogger(c.env)
  const body = await c.req.json() as { agentSlug: string; userId: string; message: string; channel?: string }
  if (!body.agentSlug || !body.userId || !body.message) return c.json(err('agentSlug, userId, message required'), 400)
  try {
    ...
    const res  = await stub.fetch(new Request('http://agent/message', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'chat', message: body.message, userId: body.userId, agentSlug: body.agentSlug, channel: body.channel ?? 'whatsapp' }) }))
    const data = await res.json() as { reply?: string }
    log.info({ agentSlug: body.agentSlug, userId: body.userId }, 'chat:http')
