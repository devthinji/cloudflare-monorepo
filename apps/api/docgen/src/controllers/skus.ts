...
import { createDb, skus, skuAgentAccess } from '../models'
...
export async function uploadSKU(c: Context<{ Bindings: DocgenWorkerEnv }>) {
  ...
  try {
    ...
    await db.insert(skus).values({
      ...
      templateType: ext, fileKey, markdownPreview, price, currency,
...
export async function listSKUs(c: Context<{ Bindings: DocgenWorkerEnv }>) {
  ...
  const filtered = rows
    ...
    .map(r => ({
      ...r,
      fieldSchema: JSON.parse(r.fieldSchema),
      conversationSteps: r.conversationSteps ? JSON.parse(r.conversationSteps) : null,
    }))
...
export async function getSKU(c: Context<{ Bindings: DocgenWorkerEnv }>) {
  ...
  return c.json(ok({
    ...row,
    fieldSchema: JSON.parse(row.fieldSchema),
    conversationSteps: row.conversationSteps ? JSON.parse(row.conversationSteps) : null,
  }))
}
